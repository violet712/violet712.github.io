...
# renders environment if running reward is greater then this threshold
...
class Actor(object):
    ...
    def learn(self, s, a, td):
        with tf.GradientTape() as tape:
            ...
            _exp_v = tl.rein.cross_entropy_reward_loss(
                logits=_logits, actions=[a], rewards=td[0])
    ...
    def choose_action_greedy(self, s):
        # logits: probability distribution of actions
        _logits = self.model(np.array([s]))
        _probs = tf.nn.softmax(_logits).numpy()
    ...
    def load_ckpt(self):  # load trained weights
        tl.files.load_and_assign_npz(
            name='model_actor.npz', network=self.model)
...
class Critic(object):
    ...
    def load_ckpt(self):  # load trained weights
        tl.files.load_and_assign_npz(
            name='model_critic.npz', network=self.model)
                ...
                if done or t >= MAX_EP_STEPS:
                    ...
                    if t >= MAX_EP_STEPS:
                        ...
                        while True:
                            env.render()
                            # a = actor.choose_action(s)
                            # Hao Dong: it is important for this task
                            a = actor.choose_action_greedy(s)
                            s_new, r, done, info = env.step(a)
                            s_new = np.concatenate(
                                (s_new[0:N_F], s[N_F:]), axis=0).astype(np.float32)
                            rall += r
                            s = s_new
                            if done:
                                print("reward", rall)
