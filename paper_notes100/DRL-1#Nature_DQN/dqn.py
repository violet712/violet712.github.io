...
class Model(DQNModel):
    ...
    def _get_DQN_prediction(self, image):
        ...
        if image.shape.rank == 5:
            # merge C & Hist
            image = tf.reshape(image, [-1] + list(self.state_shape[:2]) +
                               [self.state_shape[2] * FRAME_HISTORY])

        image = image / 255.0
        with argscope(Conv2D,
                      activation=lambda x: PReLU('prelu', x),
                      use_bias=True):
            l = (
                LinearWrap(image)
                # Nature architecture
                .Conv2D('conv0', 32, 8,
                        strides=4).Conv2D('conv1', 64, 4,
                                          strides=2).Conv2D('conv2', 64, 3)
...
def get_config(model):
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            expreplay,
            ScheduledHyperParamSetter('learning_rate', [(0, 1e-3), (60, 5e-4),
                                                        (400, 1e-4)]),
            ScheduledHyperParamSetter(
                ObjAttrParam(expreplay, 'exploration'),
                [(0, 1), (10, 0.1),
                 (400, 0.01)],  # 1->0.1 in the first million steps
                interp='linear'),
            PeriodicTrigger(Evaluator(args.num_eval, ['state'], ['Qvalue'],
                                      get_player),
